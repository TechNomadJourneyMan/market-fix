...
export function Header({ user, unreadCount }: HeaderProps) {
  ...
  // На главной шапка прозрачная, пока страница не проскроллена — hero смотрится цельно.
  ...
  const showInlineSearch = pathname !== '/' && !pathname.startsWith('/catalog');
          ...
          <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
            ...
            <SheetContent side="right" className="w-[86vw] max-w-xs">
                ...
                <SheetTitle className="flex items-center gap-3">
                  ...
                  <span className="min-w-0">
                    ...
                    <span className="block truncate text-xs font-normal text-muted-foreground">
                      {user.loyaltyPoints} бонусных баллов
                    </span>
                  </span>
                </SheetTitle>
              </SheetHeader>
              <SheetBody className="space-y-1">
                ...
                />
                <MobileLink href="/account/settings" icon={Settings} label="Настройки" onNavigate={() => setMobileOpen(false)} />
